...
class ManipulatorEnv(gym.Env):
    def __init__(
        ...
        assert (
            state_encoding in StateEncoding
        ), f"Invalid state encoding: {state_encoding}"
        if state_encoding == StateEncoding.JOINT:
            assert (
                manipulator_interface.joint_states is not None
        ...
        action_low = (
            np.ones(manipulator_interface.step_action_shape) * -eef_displacement
        )
        action_high = (
            np.ones(manipulator_interface.step_action_shape) * eef_displacement
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", help="run the environment test with base class"
    )
    parser.add_argument(
        "--client", action="store_true", help="run the environment with the client"
    )
    parser.add_argument(
        "--widowx_sim",
        action="store_true",
    ...
    parser.add_argument(
        "--host", type=str, default="localhost", help="host of the client"
    )
    parser.add_argument(
        "--log_dir", type=str, default=None, help="log the data to the directory"
    )
    ...
    env = ClipActionBoxBoundary(
        env,
        workspace_boundary=np.array([[-0.9, -9.0, -9.0], [0.35, 9.0, 0.35]]),
    )
    ...
    trunc = False
    action = np.array([0.01, 0.00, 0.01, 0.00, 0.0, 0.0, 1.0], dtype=np.float32)
